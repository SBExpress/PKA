...
import { Mail, Phone, MapPin, Edit2, ChevronDown, ChevronUp } from 'lucide-react'
import Link from 'next/link'
import CompanyForm from './forms/CompanyForm'

export default function CompanyDetail({ company, onSaved }) {
    ...
    return (
      <div className="max-w-2xl">
        ...
        <div className="bg-white rounded-xl shadow-sm p-8">
          <CompanyForm company={company} onSaved={() => { setIsEditing(false); onSaved?.(company.id) }} />
        </div>
      </div>
    )
  }
            ...
            {company.type && (
              <div className="pt-2 border-t border-slate-100">
                <p className="text-slate-600">
                  <span className="font-medium">Type:</span> {company.type.replace('_', ' ')}
                </p>
              </div>
            )}
          </div>
